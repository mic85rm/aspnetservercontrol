...
{
  public partial class BCtlsysVisibilita : BWebControlTableBase
  {
    ...
    {
      BsysVisibilita MyObj = null;
      try
      {
        if (Obj == null) return false;
        MyObj = (BsysVisibilita)Obj;
      ...
      {
        this.WriteLog("sysVisibilita.CambiaRec():", "", ex, eSeverity.ERROR);
        return false;
      }
    }
    public override BArts.BBaseClass.BBaseObject ScriviRec(BArts.BBaseClass.BBaseObject Obj)
    {
      ...
      try
      {
        ...
        if (Ret)
        {
          BSysUtenteEntrato.WriteOperation("SALVATAGGIO DEL RECORD IDENTIFICATO DALLA CHIAVE <<[ID] = " + mbtID.Text + ";>> DELLA TABELLA sysVisibilita");
          return MyObj;
        }
        else
        {
      ...
      {
        this.WriteLog("sysVisibilita.ScriviRec():", "", ex, eSeverity.ERROR);
        return null;
      }
    }
