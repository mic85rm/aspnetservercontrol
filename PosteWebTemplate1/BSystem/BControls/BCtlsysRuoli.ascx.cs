...
{
  public partial class BCtlsysRuoli : BWebControlTableBase
  {
    ...
    public override bool CambiaRec(BArts.BBaseClass.BBaseObject Obj)
    {
      ...
      {
        this.WriteLog("sysRuoli.CambiaRec():", "", ex, eSeverity.ERROR);
        return false;
      }
    }
    public override BArts.BBaseClass.BBaseObject ScriviRec(BArts.BBaseClass.BBaseObject Obj)
    {
      ...
      try
      {
        ...
        if (Ret)
        {
          BSysUtenteEntrato.WriteOperation("SALVATAGGIO DEL RECORD IDENTIFICATO DALLA CHIAVE <<[ID] = " + mbtID.Text + ";>> DELLA TABELLA sysRuoli");
          return MyObj;
        }
        else
        {
      ...
      {
        this.WriteLog("sysRuoli.ScriviRec():", "", ex, eSeverity.ERROR);
        return null;
      }
    }
