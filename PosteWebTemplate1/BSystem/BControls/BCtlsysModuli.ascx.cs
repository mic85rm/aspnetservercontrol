...
{
  public partial class BCtlsysModuli : BWebControlTableBase
  {
    ...
    {
      BsysModuli MyObj = null;
      try
      {
        if (Obj == null) return false;
        MyObj = (BsysModuli)Obj;
      ...
      {
        this.WriteLog("sysModuli.CambiaRec():", "", ex, eSeverity.ERROR);
        return false;
      }
    }
    public override BArts.BBaseClass.BBaseObject ScriviRec(BArts.BBaseClass.BBaseObject Obj)
    {
      ...
      try
      {
        ...
        if (Ret)
        {
          BSysUtenteEntrato.WriteOperation("SALVATAGGIO DEL RECORD IDENTIFICATO DALLA CHIAVE <<[ID] = " + mbtID.Text + ";>> DELLA TABELLA sysModuli");
          return MyObj;
        }
        else
        {
      ...
      {
        this.WriteLog("sysModuli.ScriviRec():", "", ex, eSeverity.ERROR);
        return null;
      }
    }
